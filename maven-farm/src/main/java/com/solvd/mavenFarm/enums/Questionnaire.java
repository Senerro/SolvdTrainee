...
public enum Questionnaire
{
    ...
    PREPARATION_TO_KILL,
    MARKET_SCENE,
    MARKET_SELL_RAW_SCENE,
    MARKET_BUY_RESOURCES_SCENE,
    MARKET_BUY_ANIMAL_SCENE,
    FARM_RAW_SCENE,
    ...
    private Questionnaire transformToEnum() {
        ...
        var charArray = this.invokeMethod.toCharArray();
        ...
        for(int i = 1; i < charList.size() - 1; i++) {
            if (charList.get(i) == Character.toUpperCase(charList.get(i)) && charList.get(i + 1) == Character.toUpperCase(charList.get(i + 1))){
                if(charList.get(i-1) != Character.toUpperCase(charList.get(i-1))){
                    charList.add(i, '_');
                }
                continue;
            }
        ...
        this.invokeMethod = StringUtils.upperCase(sb.toString());
